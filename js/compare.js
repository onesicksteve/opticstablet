...
function safeText(s) {
  ...
  function renderCompare(products) {
    ...
    const headerCols = products
      .map((p) => {
        const name = encodeHtml(productName(p));
        const hasSale = p.sale_price_gbp !== undefined && p.sale_price_gbp !== null && p.sale_price_gbp !== "";
        const price = formatGBP(p.price_gbp);
        const sale = hasSale ? formatGBP(p.sale_price_gbp) : "";
        const priceHtml = hasSale
          ? `<div class="compare-price"><span class="old-price">${encodeHtml(price)}</span><span class="sale-price">${encodeHtml(sale)}</span></div>`
          : (price ? `<div class="compare-price">${encodeHtml(price)}</div>` : "");
        return `<th><div class="compare-head"><div class="compare-name">${name}</div>${priceHtml}</div></th>`;
      })
      .join("");
